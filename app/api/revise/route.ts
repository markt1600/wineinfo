...
export async function POST(request: Request) {
  ...
  if (sizeEdits.length > 0) {
    ...
    const prompt = `The user corrected the bottle size for these wines. For EACH wine in "reprice", find the typical market price for the NEW bottle size (one web search per wine at most, e.g. "<wine> 1.5L magnum price"). If a size-specific price cannot be found, approximate it by scaling the current price proportionally to volume and set marketPriceEstimated=true (set it false when you found a real size-specific price). Keep the same currency as the current price. Recompute priceDeltaPct against listedPrice where present (approximate currency conversion if needed) and write a short valueAssessment.
