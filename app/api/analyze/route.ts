...
- Typical current retail market price. This app's home market is Singapore, so the price ladder is: (1) a Singapore retailer's price — millesima.sg and sg.cruworldwine.com are authoritative first stops, but any genuine SG shop counts — reported in SGD as found; (2) a Hong Kong retailer's price if no SG listing surfaces; (3) the Wine-Searcher-style international average only as the last resort. A search result listing BOTH an SG shop and wine-searcher must use the SG shop's price. Prices differ 2x+ between markets, so the market matters as much as the number. Only when the photo's prices are clearly in another market's currency (GBP menu → UK, EUR → that country, …) prefer that market's retailers instead. Include the appellation in the query whenever you read one (label, tag, or menu subheader) — producers often sell bottlings from several appellations at wildly different prices, and the appellation is frequently the only thing that tells them apart. NEVER use a restaurant's or bar's own wine-list price as the market price — those carry the very markup this app measures. Only independent retail, wholesale, or auction listings count; if that's all you can find, leave marketPrice null rather than pass a menu price off as market.
...
Be economical with research: at most TWO web searches per identified wine. The FIRST query must be Singapore-qualified — "<producer> <wine> <appellation> <vintage> price Singapore" — which usually surfaces SG retailers, the international aggregate, AND a Vivino/CellarTracker score together in the result snippets; take the SG price when one appears. Only if that query yields no usable price at all, ONE fallback query without the Singapore qualifier ("<producer> <wine> <vintage> price rating"). COVERAGE COMES FIRST: run the first query for EVERY identified wine before spending a fallback query on any of them — a photo where every wine has some price beats one where a few wines were researched twice and the rest not at all. Use web_fetch only when the snippets genuinely don't contain the number you need. Never search for unidentified bottles, and don't re-verify data you already have.
        ...
        const tools: unknown[] = [
          {
            ...
            name: "web_search",
            // Two queries per wine: the SG-qualified first attempt plus one
            // generic fallback (see the prompt's economy rule).
            max_uses: MAX_RESEARCH * 2,
          },
          {
